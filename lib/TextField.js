  ...
  render() {
    ...
    return (
      <View style={[dense ? styles.denseWrapper : styles.wrapper, this.state.height ? {height: undefined}: {}, wrapperStyle]} ref="wrapper">
          <View>
        <TextInput
          style={[textInputComposedStyles, (secureTextEntry && secureTextAllowUnmask) ? {paddingRight: 34}: {}]}
          multiline={multiline}
          onFocus={() => {
        ...
        />
        {
          secureTextEntry && secureTextAllowUnmask ?
              <TouchableWithoutFeedback onPress={this.toggleSecureMask.bind(this)}>
                <View style={[styles.secureTextToggleIcon, {opacity: !this.state.text.length ? 0.38 : 0.54}]}>{!this.state.isUnmasked ? secureTextAllowUnmaskIconOn : secureTextAllowUnmaskIconOff}</View>
